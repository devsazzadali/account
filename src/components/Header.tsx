                ...
                <div className="relative group">
                    <button className="flex items-center gap-3 focus:outline-none py-2 px-1">
                        <div className="hidden md:block text-right">
                            ...
                            <div className="text-[10px] text-primary-600 font-medium tracking-wider uppercase">
                                {isAdmin ? "Administrator" : "Member"}
                            </div>
                        </div>
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary-500 to-blue-600 flex items-center justify-center text-white font-bold shadow-lg shadow-primary-500/20 border-2 border-white ring-2 ring-transparent group-hover:ring-primary-500/30 transition-all">
                        {username.charAt(0).toUpperCase()}
                        </div>
                    </button>

                    {/* Dropdown Menu */}
                    <div className="absolute right-0 top-full mt-2 w-72 bg-white rounded-xl border border-slate-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 transform origin-top-right translate-y-2 group-hover:translate-y-0 overflow-hidden shadow-2xl">
                        {/* Header Section */}
                        <div className="p-5 border-b border-slate-100 bg-slate-50/50">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="font-bold text-slate-900 mb-0.5">{username}</div>
                        ...
                        <div className="p-2 space-y-1 bg-white">
                            ...
                            {isAdmin ? (
                                <>
                                    <DropdownItem to="/seller/orders" icon={<Store size={18} className="text-primary-600" />} label="Store Management" />
                                    <DropdownItem to="/admin" icon={<Gavel size={18} className="text-orange-500" />} label="Dispute Center" />
                                    <DropdownItem to="/admin" icon={<Ticket size={18} className="text-blue-500" />} label="Service Tickets" />
                                    <div className="h-px bg-slate-100 my-2 mx-2"></div>
                                    <DropdownItem to="/admin" icon={<Settings size={18} className="text-slate-500" />} label="System Settings" />
                                </>
                            ) : (
                                <>
                                    <DropdownItem to="/dashboard" icon={<ShoppingBag size={18} className="text-primary-600" />} label="My Orders" />
                                    <DropdownItem to="/dashboard" icon={<Ticket size={18} className="text-blue-500" />} label="Support Tickets" />
                                    <div className="h-px bg-slate-100 my-2 mx-2"></div>
                                    <DropdownItem to="/dashboard" icon={<Settings size={18} className="text-slate-500" />} label="Account Settings" />
                                </>
                            )}
                        </div>
                        ...
                        <div className="p-3 border-t border-slate-100 bg-slate-50/50">
                            <button 
                                ...
                                className="w-full bg-red-50 hover:bg-red-500 hover:text-white text-red-600 text-sm font-semibold py-2.5 rounded-lg flex items-center justify-center gap-2 transition-all duration-200"
                            >
                                <LogOut size={16} />
                                Sign Out
                            </button>
...
function DropdownItem({ to, icon, label }: { to: string, icon: React.ReactNode, label: string }) {
    return (
        <Link to={to} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-all group">
